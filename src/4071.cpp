//
// 4071.cpp for nano in /home/kotra/rendu/cpp_nanotekspice/src/
//
// Made by Kotra
// Login   <mathis.rousselot@epitech.eu@epitech.eu>
//
// Started on  Thu Mar  2 13:53:48 2017 Kotra
// Last update Sun Mar  5 04:40:49 2017 Kotra
//

#include "all_chips.hpp"

C4071::C4071(std::string info) : chipset(info)
{
...
nts::Tristate C4071::Compute(size_t num)
{
	...
	if (num == 11 || num == 4)
	{
		...
		return (_pins[num].state = or_enum(VALUEPIN(num + 1), VALUEPIN(num + 2)));
	}
	if (num == 10 || num == 3)
	{
		VALUEPIN(num - 1) = this->Compute(num - 1);
		VALUEPIN(num - 2) = this->Compute(num - 2);
		return (_pins[num].state = or_enum(CPTFROM(num - 1), CPTFROM(num - 2)));
	}
